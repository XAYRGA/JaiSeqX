...
namespace jaudio
{
    internal partial class BinaryAudioArchive : AudioArchive
    {
        ...
        public BinaryAudioArchive() : base()
        {
            Type = AudioArchiveType.BAA;
        }

        public static new BinaryAudioArchive CreateFromStream(bgReader rd)
        {
        ...
        private int baa_GetSectionSize(AudioArchiveSectionInfo sect, bgReader br)
        {
            switch (sect.type)
            {
...
#if DEBUG
                Console.WriteLine($"baa: Load section {type}@0x{offset:X}[{size}]");
#endif
...
#if DEBUG
                Console.WriteLine($"baa: \t {sect.type}@0x{sect.offset:X} Size={sect.size:X}");
#endif
            }
        }
    }
